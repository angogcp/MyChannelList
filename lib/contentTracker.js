...
function createContentTracker(options = {}) {
  ...
  async function updatePreferences({
    ...
    autoQueueThreshold,
    autoQueueMode,
    autoQueueUploadToDrive,
    autoQueueDriveFolderId,
    autoQueueDeleteLocalAfterUpload
  } = {}) {
    const store = await loadStore();
...
function normalizePreferences(input = {}) {
  ...
  const autoQueueThreshold = input.autoQueueThreshold !== undefined
    ? clampNumber(input.autoQueueThreshold, 0, 100, 0)
    : 0;
  const autoQueueMode = ["video", "audio"].includes(String(input.autoQueueMode || ""))
    ? String(input.autoQueueMode)
    : "video";
  const autoQueueUploadToDrive = !!input.autoQueueUploadToDrive;
  const autoQueueDriveFolderId = String(input.autoQueueDriveFolderId || "").trim();
  const autoQueueDeleteLocalAfterUpload = !!input.autoQueueDeleteLocalAfterUpload;
