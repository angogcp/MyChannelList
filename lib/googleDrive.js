...
function createGoogleDriveManager(options = {}) {
  ...
  async function listFolders() {
    ...
    const url = `https://www.googleapis.com/drive/v3/files?pageSize=100&orderBy=name_natural&fields=files(id,name)&q=${query}&supportsAllDrives=true&includeItemsFromAllDrives=true`;
    ...
    return {
      ...
      folders: [
        ...
        ...((response.files || []).map((folder) => ({ id: String(folder.id), name: String(folder.name) })))
      ]
    };
  }
