function createLlmAnalyzer(options = {}) {
  ...
  async function analyzeContent(input = {}) {
    if (!apiKey) {
      throw new Error("AI analysis is not configured. Set DEEPSEEK_API_KEY on the server.");
    }
    ...
    if (!response.ok) {
      const message = payload?.error?.message || "AI analysis request failed.";
...
function extractOutputText(payload) {
  // OpenAI /responses format (legacy)
  ...
  // Chat completions format (OpenAI-compatible: DeepSeek, etc.)
