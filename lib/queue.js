...
function createDownloadQueue(options = {}) {
  ...
  const contentTracker = options.contentTracker || null;
  ...
  function addJob(payload) {
    ...
    const job = {
      ...
      trackerItem: payload.trackerItem || null,
    ...
    jobs.set(id, job);
    if (contentTracker && job.trackerItem) {
      contentTracker.markQueued(job.trackerItem).catch((err) => {
        console.error("Failed to mark content as queued:", err);
      });
    }
    broadcast();
    processNext();
  ...
  async function processNext() {
    ...
    try {
      const result = await downloader.download({
        ...
        quality: job.quality,
        onProgress: (prog) => {
          if (prog.type === "progress" || prog.type === "bootstrap") {
            if (Number.isFinite(prog.percent)) {
              job.percent = prog.percent;
            }
            job.message = prog.message || job.message;
            ...
            if (prog.filePath && !job.filePath) {
              job.filePath = prog.filePath;
              job.fileName = path.basename(prog.filePath);
              if (!job.title) {
      ...
      if (contentTracker && job.trackerItem) {
        await contentTracker.markDownloaded(job.trackerItem);
      }
      ...
      if (contentTracker && job.trackerItem) {
        await contentTracker.markFailed(job.trackerItem, job.message);
      }
    } finally {
